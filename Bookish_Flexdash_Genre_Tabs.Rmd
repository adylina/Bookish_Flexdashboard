...
# import the kraken 
TBR <- read_sheet("google_sheets_url_goes_here")
...
```{r eval=TRUE, echo=FALSE, error=FALSE, message=FALSE, warning=FALSE, fig.dim = c(11, 6)}
...
ggraph(bigram_graph, layout = "fr") +
  ...
  geom_node_text(aes(label = name), repel = T, vjust = .5, hjust = .5, size = 4.5) +
  theme_void()
...
TBR %>%
  ...
  select(c(Author, Series, Title, Genre)) %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover", "responsive", "condensed")) %>%
  column_spec(1, width = "15em") %>%
  column_spec(2, width = "15em") %>%
...
### TBR list: series books

```{r eval=TRUE, echo=FALSE, error=FALSE, message=FALSE, warning=FALSE}

TBR %>%
  filter((is.na(Start_Finish)) & (is.na(Start_DNF))) %>%
  filter(Series != "None") %>%
  group_by(Author, Series, Genre) %>%
  count(Title) %>%
  summarise(sum(n, na.rm = T)) %>%
  rename(`Unread Books in Series` = "sum(n, na.rm = T)") %>%
  select(c(Author, Series, Genre, `Unread Books in Series`)) %>%
...
Books I Read {data-navmenu="Reading Overview"}
=====================================
...
TBR %>%
  ...
  filter(Series != "None") %>%
  group_by(Author, Series, Genre) %>%
  distinct(Title, Rating) %>%
  summarise(n_distinct(Title), mean(Rating)) %>%
  rename(`Mean Rating` = "mean(Rating)", `Books Read in Series` = "n_distinct(Title)") %>%
  mutate(`Mean Rating` = as.numeric(`Mean Rating`)) %>%
  mutate_if(is.numeric, round, 1) %>%
  select(c(Author, Series, Genre, `Mean Rating`, `Books Read in Series`)) %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover", "responsive", "condensed")) %>%
...
TBR %>%
  filter(Start_Finish == "1") %>%
  filter(Series == "None") %>%
  group_by(Author, Genre, Rating) %>%
  distinct(Title) %>%
  select(c(Author, Title, Genre, Rating)) %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover", "responsive", "condensed")) %>%
  column_spec(1, width = "10em") %>%
...
TBR %>%
  ...
  summarise(sum(Start_Finish, na.rm = T)) %>%
  rename(`Books Read` = "sum(Start_Finish, na.rm = T)") %>%
  ggplot(aes(fill = Year, x = Genre, y = `Books Read`)) +
  geom_bar(stat="identity") + 
  scale_colour_discrete(name = "Year") +
  expand_limits(y = 0) +
    labs(title = "Count of Books Read Each Year by Genre",
         #       caption = "Source: Amanda's Reading Log",
         y = "Number of Books Read",
         x = "Genre") +
  theme_classic() +
  theme(strip.background = element_rect(fill = "#FFFFFF"),
        ...
        axis.text.x = element_text(angle = 45, size = 10, vjust = 1.0, hjust = 1.0),
...
TBR %>%
  ...
  group_by(Year, Genre) %>%
  summarise(sum(Start_DNF, na.rm = T)) %>%
  rename(`Books Read` = "sum(Start_DNF, na.rm = T)") %>%
  ggplot(aes(fill = Year, x = Genre, y = `Books Read`)) +
  geom_bar(stat="identity") + 
  scale_colour_discrete(name = "Year") +
  expand_limits(y = 0) +
...
Single Day Reads {data-navmenu="Reads"}
=====================================
...
bigrams <- subset(TBR, Single_Day == "1")
...
TBR %>%
  filter((Single_Day == "1") & (!is.na(Month)) & (Start_Finish == "1")) %>%
  group_by(Year, Month) %>%
  summarise(sum(Start_Finish)) %>%
  rename(`Books Read` = "sum(Start_Finish)") %>%
  ggplot(aes(x = Month, y = `Books Read`, group = Year)) +
...
Multi Day Reads {data-navmenu="Reads"}
=====================================
...
bigrams <- subset(TBR, Single_Day == "0")
...
TBR %>%
  filter((Single_Day == "0") & (!is.na(Month)) & (Start_Finish == "1")) %>%
  group_by(Year, Month) %>%
  summarise(sum(Start_Finish)) %>%
  rename(`Books Read` = "sum(Start_Finish)") %>%
  ggplot(aes(x = Month, y = `Books Read`, group = Year)) +
...
Overall Genre Trends {data-navmenu="Genres"}
=====================================
...
TBR %>%
  filter(Start_Finish == "1") %>%
  group_by(Year, Month, Fiction) %>%
  summarise(sum(Start_Finish, na.rm = T)) %>%
  rename(`Books Read` = "sum(Start_Finish, na.rm = T)") %>%
  ggplot(aes(fill = Year, x = Month, y = `Books Read`)) +
  geom_bar(stat="identity") + 
  scale_colour_discrete(name = "Year") +
  expand_limits(y = 0) +
    labs(title = "Count of Books Read Each Year by Fiction or Non-Fiction Status",
         #       caption = "Source: Amanda's Reading Log",
         y = "Number of Books Read",
         x = "Month") +
  facet_wrap(~Fiction) +
...
TBR %>%
  filter(Start_Finish == "1") %>%
  group_by(Year, Subgenre) %>%
  summarise(sum(Start_Finish, na.rm = T)) %>%
  rename(`Books Read` = "sum(Start_Finish, na.rm = T)") %>%
  ggplot(aes(fill = Year, x = Subgenre, y = `Books Read`)) +
  geom_bar(stat="identity") + 
  scale_colour_discrete(name = "Year") +
  expand_limits(y = 0) +
    labs(title = "Count of Books Read Each Year by Genre",
...
TBR %>%
  ...
  summarise(sum(Pages, na.rm = T)) %>%
  rename(`Pages Read` = "sum(Pages, na.rm = T)") %>%
  ggplot(aes(fill = Year, x = Month, y = `Pages Read`)) +
  geom_bar(stat="identity") + 
  scale_colour_discrete(name = "Year") +
  expand_limits(y = 0) +
    labs(title = "Count of Books Read Each Year by Fiction or Non-Fiction Status",
...
bigrams <- subset(TBR, (Genre == "Fantasy") & (Start_Finish == "1"))
...
TBR %>%
  filter((!is.na(Month)) & (Start_Finish == "1") & (Genre == "Fantasy")) %>%
  group_by(Year, Month) %>%
  summarise(sum(Start_Finish)) %>%
  rename(`Books Read` = "sum(Start_Finish)") %>%
  ggplot(aes(x = Month, y = `Books Read`, group = Year)) +
...
-----------------------------------------------------------------------
  ...
  ### Number of reads by subgenre
  
  ```{r eval=TRUE, echo=FALSE, error=FALSE, message=FALSE, warning=FALSE, fig.dim = c(11, 6)}

TBR %>%
  filter((Start_Finish == "1") & (Genre == "Fantasy")) %>%
  group_by(Year, Subgenre) %>%
  summarise(sum(Start_Finish, na.rm = T)) %>%
  rename(`Books Read` = "sum(Start_Finish, na.rm = T)") %>%
  ggplot(aes(fill = Year, x = Subgenre, y = `Books Read`)) +
  ...
  scale_colour_discrete(name = "Year") +
  expand_limits(y = 0) +
  labs(title = "Count of Books Read Each Year by Subgenre",
       #       caption = "Source: Amanda's Reading Log",
       y = "Number of Books Read",
       x = "Genre") +
  theme_classic() +
...
TBR %>%
  filter((Start_Finish == "1") & (Genre == "History")) %>%
  group_by(Year, Subgenre) %>%
  summarise(sum(Start_Finish, na.rm = T)) %>%
  rename(`Books Read` = "sum(Start_Finish, na.rm = T)") %>%
  ggplot(aes(fill = Year, x = Subgenre, y = `Books Read`)) +
...
bigrams <- subset(TBR, (Genre == "Mystery") & (Start_Finish == "1"))
...
TBR %>%
  filter((!is.na(Month)) & (Start_Finish == "1") & (Genre == "Mystery")) %>%
  group_by(Year, Month) %>%
  summarise(sum(Start_Finish)) %>%
  rename(`Books Read` = "sum(Start_Finish)") %>%
  ggplot(aes(x = Month, y = `Books Read`, group = Year)) +
...
TBR %>%
  filter((Start_Finish == "1") & (Genre == "Mystery")) %>%
  group_by(Year, Subgenre) %>%
  summarise(sum(Start_Finish, na.rm = T)) %>%
  rename(`Books Read` = "sum(Start_Finish, na.rm = T)") %>%
  ggplot(aes(fill = Year, x = Subgenre, y = `Books Read`)) +
...
bigrams <- subset(TBR, (Genre == "Poetry") & (Start_Finish == "1"))
...
TBR %>%
  filter((Rating == "5") & (!is.na(Month)) & (Start_Finish == "1") & (Genre == "Poetry")) %>%
  group_by(Year, Month) %>%
  summarise(sum(Start_Finish)) %>%
  rename(`Books Read` = "sum(Start_Finish)") %>%
  ggplot(aes(x = Month, y = `Books Read`, group = Year)) +
...
TBR %>%
  filter((Start_Finish == "1") & (Genre == "Poetry")) %>%
  group_by(Year, Subgenre) %>%
  summarise(sum(Start_Finish, na.rm = T)) %>%
  rename(`Books Read` = "sum(Start_Finish, na.rm = T)") %>%
  ggplot(aes(fill = Year, x = Subgenre, y = `Books Read`)) +
...
bigrams <- subset(TBR, (Genre == "Romance") & (Start_Finish == "1"))
...
TBR %>%
  filter((!is.na(Month)) & (Start_Finish == "1") & (Genre == "Romance")) %>%
  group_by(Year, Month) %>%
  summarise(sum(Start_Finish)) %>%
  rename(`Books Read` = "sum(Start_Finish)") %>%
  ggplot(aes(x = Month, y = `Books Read`, group = Year)) +
...
TBR %>%
  filter((Start_Finish == "1") & (Genre == "Romance")) %>%
  group_by(Year, Subgenre) %>%
  summarise(sum(Start_Finish, na.rm = T)) %>%
  rename(`Books Read` = "sum(Start_Finish, na.rm = T)") %>%
  ggplot(aes(fill = Year, x = Subgenre, y = `Books Read`)) +
...
bigrams <- subset(TBR, (Genre == "SciFi") & (Start_Finish == "1"))
...
TBR %>%
  filter((!is.na(Month)) & (Start_Finish == "1") & (Genre == "SciFi")) %>%
  group_by(Year, Month) %>%
  summarise(sum(Start_Finish)) %>%
  rename(`Books Read` = "sum(Start_Finish)") %>%
  ggplot(aes(x = Month, y = `Books Read`, group = Year)) +
...
TBR %>%
  filter((Start_Finish == "1") & (Genre == "SciFi")) %>%
  group_by(Year, Subgenre) %>%
  summarise(sum(Start_Finish, na.rm = T)) %>%
  rename(`Books Read` = "sum(Start_Finish, na.rm = T)") %>%
  ggplot(aes(fill = Year, x = Subgenre, y = `Books Read`)) +
...
Young Adult {data-navmenu="Genres"}
=====================================
...
bigrams <- subset(TBR, (Genre == "Young Adult") & (Start_Finish == "1"))
...
TBR %>%
  filter((!is.na(Month)) & (Start_Finish == "1") & (Genre == "Young Adult")) %>%
  group_by(Year, Month) %>%
  summarise(sum(Start_Finish)) %>%
  rename(`Books Read` = "sum(Start_Finish)") %>%
  ggplot(aes(x = Month, y = `Books Read`, group = Year)) +
...
TBR %>%
  filter((Start_Finish == "1") & (Genre == "Young Adult")) %>%
  group_by(Year, Subgenre) %>%
  summarise(sum(Start_Finish, na.rm = T)) %>%
  rename(`Books Read` = "sum(Start_Finish, na.rm = T)") %>%
  ggplot(aes(fill = Year, x = Subgenre, y = `Books Read`)) +
